...
if _triton_available:
    ...
    def _quant_kernel(
        ...
        k_fp16_ptrs = (
            ...
            + pid_p * stride_kp_in
            + pid_h * stride_kh_in
            + offs_s[:, None] * stride_ks_in
            + offs_d[None, :] * stride_kd_in
        )
        k = tl.load(k_fp16_ptrs, mask=offs_s[:, None] < PAGE_SIZE, other=0.0).to(
            tl.float32
        )
        ...
        v_fp16_ptrs = (
            ...
            + pid_p * stride_vp_in
            + pid_h * stride_vh_in
            + offs_s[:, None] * stride_vs_in
            + offs_d[None, :] * stride_vd_in
        )
        v = tl.load(v_fp16_ptrs, mask=offs_s[:, None] < PAGE_SIZE, other=0.0).to(
            tl.float32
        )
...
def _compute_selected_page_ids(
    ...
    bt = BlockTable(block_size=page_size)
    pages, _ = bt.create_block_table(selected_layout, kv_len)
    return _unique_preserve_order(pages).to(dtype=torch.int32)
...
def semantic_block_attention_triton(
    ...
    if prefetch and _can_run_gpu_kernel():
        ...
        current_pages = _compute_selected_page_ids(
            layout, kv_tokens, threshold=threshold
        )
