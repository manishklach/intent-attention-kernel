...
def quantise_kv_page(
    ...
    k_int8 = torch.clamp(torch.round(k.float() / scale_k.float().unsqueeze(0)), -127, 127).to(torch.int8)
    v_int8 = torch.clamp(torch.round(v.float() / scale_v.float().unsqueeze(0)), -127, 127).to(torch.int8)
...
def quantise_kv_cache(
    ...
    for p in range(num_pages):
        ...
        k_pad = torch.zeros(batch, heads, page_size, d_head, dtype=k.dtype, device=k.device)
        v_pad = torch.zeros(batch, heads, page_size, d_head, dtype=v.dtype, device=v.device)
